...
addpath(genpath('Matlab-toolkit\EPANET-Matlab-Toolkit-master'))
start_toolkit; %to start the epanet toolkit
%%
% Load a network
d = epanet(['araujo.inp']); %importa data from .inp file

%%

type="PDA";   %type of analysis pressure driven or demand driven
...
hrs = 24; %total hours of analysis
...
while (tstep>0)                       %condition to run simulations each hour until the time simulation duration is reached (tstep=1)
    ...
    H=[H; d.getNodeHydaulicHead];     % get the node hydraulic head calculated in the nodes for the step
...
d.plot('nodes', 'yes', 'fontsize', 12)  %plot the layout of the network
